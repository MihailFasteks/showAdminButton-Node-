...
module.exports = {
	...
	getAllItems: function (req, res, isAdmin) {
		...
		request.stream = true;
		request.query("SELECT * FROM Test1");
		...
		request.on('done', function (affected) {
			...
			if (req.url == '/') {
				res.render('index', { data: self.tableRows, buttons: false, isAdmin: isAdmin });
			} else {
				res.render('index', { data: self.tableRows, buttons: true, isAdmin: isAdmin });
			}
		})
	...
	insertItem: function (data, req, res) {
		...
		ps.prepare("INSERT INTO Test1 (name, description, completed) VALUES (@name , @description, @completed)", function (err) {
	...
	loadItemById: function (req, res) {
		...
		ps.prepare('SELECT * FROM Test1 WHERE id=@id', function (err) {
	...
	updateItem: function (req, res) {
		...
		ps.prepare("UPDATE Test1 SET name=@name, description=@description, completed=@completed WHERE id=@id", function (err) {
			if (err) console.log(err)
	...
	deleteItem: function (req, res) {
		...
		ps.prepare('DELETE FROM Test1 WHERE id=@id', function (err) {
			if (err) console.log(err)
